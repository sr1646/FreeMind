...
public enum Task {
    ...
    COPY_ALL_FILES_TO_FOLDER("Created for gcam potrait photo where it is creating folder for each photo.this program COPY all files from one folder to another folder.(so it will COPY all files from folder that means if that source folder have multiple folder inside folder still it will COPY only file from them"),
    DEVIDE_DATA_FOR_DVD("For creating dvd you can create your own custom size dvd/ here manually change variable to store instead of moving"),
    PROCESS_STORED_DVD_DATA("For creating dvd from already stored dvd data"),
    DVD_PART_TO_FOLDER("For Converting back DVD part into original folder, here we will provide source and destination that may be different then actual created by program"),
    DVD_PART_OUTPUT_TO_SOURCE("For Converting back DVD part into original folder, we will put output data to original location again."),
    DVD_PART_OUTPUT_TO_BACKUP_DONE_FOLDER("For Converting back DVD part into original folder, we will put output data to backup done location."),
    ...
    PROFIT_CALCULATOR_CUMULATIVE_FOR_DAILY_EARNING("Predict profit if earned daily for storck market"),
    READ_LIST_FROM_JSON_FILE("Read List from josn file and playwith it in program")
    ;
